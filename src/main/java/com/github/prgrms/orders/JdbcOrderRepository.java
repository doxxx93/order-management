...
public class JdbcOrderRepository implements OrderRepository {
    ...
    public List<Orders> findAll(Pageable pageable) {
        ...
        List<Orders> orders = jdbcTemplate.query(
                "SELECT * FROM orders ORDER BY seq DESC LIMIT " + pageable.getSize() + " OFFSET "
                        + pageable.getOffset(),
                mapper
        );
    ...
    public void updateStateToRejected(Long seq, String rejectMsg) {
        jdbcTemplate.update("UPDATE orders SET state = ?,reject_msg = ?, rejected_at = ? WHERE seq = ?",
                StateType.REJECTED.toString(), rejectMsg, timestampOf(LocalDateTime.now()), seq);
    }
    ...
    public void updateStateToComplete(Long seq) {
        jdbcTemplate.update("UPDATE orders SET state = ?, completed_at = ? WHERE seq = ?",
                StateType.COMPLETED.toString(), timestampOf(LocalDateTime.now()), seq);
    }
