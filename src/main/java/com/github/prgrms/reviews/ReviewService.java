...
public class ReviewService {
    ...
    public Long newReview(Long userSeq, Long orderSeq, String content) {
        ...
        Orders orders = orderService.findById(orderSeq)
                .orElseThrow(() -> new NotFoundException("Could not found order for " + orderSeq));

        checkArgument(orders.getReviewSeq() == 0, "Could not write review for order %d because have already written",
                orderSeq);
        checkArgument(orders.getState().equals(StateType.COMPLETED.toString()),
                "Could not write review for order %d because state(%s) is not allowed", orderSeq,
                orders.getState());
